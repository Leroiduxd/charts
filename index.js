...
// Cache structure : key = `${pair}-${interval}`, value = { timestamp, data }
const cache = new Map();
...
app.get('/history', async (req, res) => {
  ...
  try {
    ...
    cache.set(cacheKey, { timestamp: now, data }); // ✅ Update cache
    res.json(data);
  } catch (err) {
    console.error(err);
    if (cached) {
      console.warn("⚠️ Serving stale data from cache due to error");
      return res.json(cached.data); // ✅ Serve old data if available
    }
